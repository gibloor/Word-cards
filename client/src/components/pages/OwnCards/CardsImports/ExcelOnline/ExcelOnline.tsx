...
const ExcelOnline = () => {
  ...
  const Y = async () => {
    const readTableData = async (accessToken: Promise<any>, fileId: string, tableName: string) => {
      try {
        const response = await axios.get(`https://graph.microsoft.com/v1.0/drives/${fileId}/items/${fileId}/workbook/tables/${tableName}/rows`, {
          headers: {
